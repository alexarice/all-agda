      ...
      tests = {
        "agda-tests" = {
          depends = [
            ...
            (hsPkgs."uri-encode" or (errorHandler.buildDepError "uri-encode"))
            ] ++ (pkgs.lib).optional (system.isWindows) (hsPkgs."tasty-silver" or (errorHandler.buildDepError "tasty-silver"));
          build-tools = [
            (hsPkgs.buildPackages.Agda.components.exes.agda or (pkgs.buildPackages.agda or (errorHandler.buildToolDepError "Agda:agda")))
            ];
          buildable = true;
          modules = [
            "Bugs/Tests"
